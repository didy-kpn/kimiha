...
impl std::fmt::Display for BenchEvent {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            BenchEvent::Event1 => write!(f, "Event1"),
            BenchEvent::Event2 => write!(f, "Event2"),
            BenchEvent::Event3 => write!(f, "Event3"),
        }
    }
}
...
fn create_bench_event_bus() -> EventBus<BenchEvent> {
    let configs = vec![
        (
            BenchEvent::Event1,
            ChannelConfig::new(10, "Bench Channel 1".to_string()),
        ),
        (
            BenchEvent::Event2,
            ChannelConfig::new(10, "Bench Channel 2".to_string()),
        ),
        (
            BenchEvent::Event3,
            ChannelConfig::new(10, "Bench Channel 3".to_string()),
        ),
    ];
...
fn bench_task_scaling(c: &mut Criterion) {
    ...
    for task_count in [1, 10, 100, 1000].iter() {
        group.bench_with_input(
            BenchmarkId::new("background_tasks", task_count),
            task_count,
            |b, &count| {
                b.iter(|| {
                    let rt = Runtime::new().unwrap();
                    rt.block_on(async {
                        ...
                        for i in 0..count {
                            let task = Arc::new(Mutex::new(BenchBackgroundTask::new(
                                &format!("Bench Background Task {}", i),
                                1,
                            )));
        ...
        group.bench_with_input(
            BenchmarkId::new("event_tasks", task_count),
            task_count,
            |b, &count| {
                b.iter(|| {
                    let rt = Runtime::new().unwrap();
...
fn bench_start_shutdown(c: &mut Criterion) {
    ...
    for task_count in [1, 5, 10].iter() {
        group.bench_with_input(
            BenchmarkId::new("start", task_count),
            task_count,
            |b, &count| {
                b.iter(|| {
                    let rt = Runtime::new().unwrap();
        ...
        group.bench_with_input(
            BenchmarkId::new("shutdown", task_count),
            task_count,
            |b, &count| {
                b.iter(|| {
                    let rt = Runtime::new().unwrap();
                ...
                b.iter(|| {
                    ...
                    rt.block_on(async {
                        ...
                        let sender = scheduler
                            .event_bus()
                            .clone_sender(&BenchEvent::Event1)
                            .unwrap();
